...
const UserProfilePage = () => {
  ...
  const getUserRoleDisplay = () => {
    if (thisUser?.role === "admin") return "Admin";
    return thisUser?.role === "employer" ? "Employer" : "Job Seeker";
  };

  const getRoleBadgeClass = () => {
    if (thisUser?.role === "admin") return "bg-red-100 text-red-700";
    if (thisUser?.role === "employer") return "bg-orange-100 text-orange-700";
    return "bg-blue-100 text-blue-700";
  };

  // Shared Profile Content Component - 100% original layout preserved
  const ProfileContent = () => (
    <>
        ...
        <div className="px-6 pb-6">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between -mt-6 mb-4 gap-4">
            <div className="flex flex-col items-start sm:flex-row sm:items-end z-10 gap-2">
              <img
                src={thisUser?.avatar || "/default.png"}
                alt={thisUser?.name}
              ...
              <div className="pb-1">
                ...
                <p className="text-xs text-gray-500">{thisUser?.email}</p>
                <p
          ...
          <div className="flex flex-col gap-2">
            <div className="flex flex-wrap items-center gap-4 mt-2 border-t border-gray-100 pt-4">
              ...
              {thisUser?.facebookLink && (
                <a
                  href={thisUser.facebookLink}
                  target="_blank"
              ...
              {thisUser?.instagramLink && (
                <a
                  href={thisUser.instagramLink}
                  target="_blank"
                  rel="noopener noreferrer"
            ...
            {thisUser?.role === "employer" && (
              <div className="flex flex-col items-start sm:flex-row sm:items-center gap-2">
                <img
                  src={thisUser.companyLogo || "/default.png"}
                  alt={thisUser.companyName}
                  className="w-24 h-24 rounded-2xl object-contain bg-white shadow-sm"
                />
                <p className="text-2xl font-semibold text-gray-900 tracking-tight flex items-center justify-center gap-2">
                  {thisUser?.companyName || "N/A"}
                  {thisUser.role === "employer" &&
                    thisUser.isCompanyVerified && (
                      <span
                ...
                {thisUser.role === "employer" &&
                  thisUser.companyWebsiteLink && (
                    <a
                      href={thisUser.companyWebsiteLink}
                      target="_blank"
                      rel="noopener noreferrer"
          ...
          {thisUser?.role === "employer" && thisUser.companyDescription && (
            <div className="mt-4 pt-4 border-t border-gray-100">
              <p className="text-sm text-justify text-gray-700">
                {thisUser.companyDescription}
              </p>
            </div>
          )}
        </div>
      ...
      <div
        className={`grid grid-cols-1 ${thisUser?.role === "jobSeeker" ? "lg:grid-cols-3" : "lg:grid-cols-2"} gap-6`}
      >
            ...
            <div className="overflow-x-auto">
              <div className="flex min-w-max">
                {tabs.map((tab) => {
                  ...
                  if (thisUser?.role === "employer" && tab.id !== "about")
                    return null;
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {[
                    ...
                    {
                      label:
                        thisUser?.role === "jobSeeker"
                          ? "Location"
                          : "Company Location",
                      value:
                        thisUser?.role === "jobSeeker"
                          ? thisUser.location
                          : thisUser.companyLocation,
                    },
                    {
                      label: "Account Type",
            ...
            {activeTab === "skills_and_education" && (
              <div className="space-y-8">
                ...
                <section>
                  ...
                  {thisUser?.skills?.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {thisUser.skills.map((skill, i) => (
                        <span
                          key={i}
                ...
                <section>
                  ...
                  {thisUser?.education?.length > 0 ? (
                    <div className="space-y-3">
                      {thisUser.education.map((edu, i) => (
                        <TimelineCard key={i}>
            ...
            {activeTab === "experience" && (
              <div className="space-y-4">
                ...
                {thisUser?.experience?.length > 0 ? (
                  <div className="space-y-3">
                    {thisUser.experience.map((exp, i) => (
                      <TimelineCard key={i}>
            ...
            {activeTab === "certifications" && (
              <div className="space-y-4">
                ...
                {thisUser?.certifications?.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {thisUser.certifications.map((cert, i) => (
                      <div
                        key={i}
              ...
              {thisUser.role === "jobSeeker" && (
                <>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {thisUser?.resume && thisUser.resume !== "" && (
                      <a
                        href={thisUser.resume}
                        target="_blank"
                        rel="noopener noreferrer"
  ...
  if (me.role === "jobSeeker" && isLoading) {
    return (
  ...
  if (me.role === "jobSeeker" && thisUser.role === "admin") {
    return (
          ...
          <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/60 overflow-hidden border border-gray-100">
            <div className="px-8 pb-10 text-center">
              ...
              <div className="space-y-1 mb-6">
                ...
                <p className="text-gray-500 font-medium">{thisUser.email}</p>
              </div>
